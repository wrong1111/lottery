<template>
	<view class="box">
		...
		<nav-bar :title="'双色球'" :back="true"></nav-bar>
	...
	export default {
		...
		methods: {
			...
			betting() {
				...
				place(data, "24",this.issueNo).then(res => {
					if (res.success) {
						uni.showToast({
							title: '下单成功',
							icon: 'none'
			...
			calculation(obj) {
				...
				if (data.length > 0) {
					...
					uni.setStorageSync("ssq", this.placeData)
