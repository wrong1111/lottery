<template>
	<view class="box">
		...
		<cmd-nav-bar back title="七乐彩" font-color="#fff" background-color="#FF3F43" right-text="七乐彩开奖"
			@rightText="rightBtn">
		</cmd-nav-bar>
		<div style="height:100%">
			<p class="fc_index">第{{issueNo}}期，每周一、三、五 21:15开奖</p>
			<div class="fc">
				<view
					style="display: flex;justify-content: space-between;align-items: center;width: 95%;margin: 0 auto;">
					<p class="tips">一等奖<span>销售总额的50%（1%调整基金）</span></p>
				...
				<u-divider text="选号区(至少7个)"></u-divider>
				<ul>
					<li @click="check(1,1,index)" v-for="(item,index) in  shi" :class="item.active?'active':''">
						...
						<view style="display: flex;justify-content: center;align-items: center;">
							...
							<u-checkbox-group shape="square" @change="checkboxChange($event,index,1)"
								:disabled="redLength>=6&&item.isGallbladder==false">
	...
	export default {
		...
		methods: {
			...
			checkboxChange(item, index, type) {
				...
				if(!this.shi[index].active && !this.checkOverMaxNumber()){
					return
				}
				if (type == 1) {
			...
			randomSelect() {
				this.clear()
				let numberArr = this.globalUtil.randomFromZero(31, 7);
