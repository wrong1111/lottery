<template>
	<view class="box">
		...
		<nav-bar :title="'福彩3D'" :back="true"></nav-bar>
	...
	export default {
		...
		methods: {
			...
			betting() {
				...
				place(data, "21",this.issueNo).then(res => {
					if (res.success) {
						uni.showToast({
							title: '下单成功',
							icon: 'none'
			...
			calculation(obj) {
				...
				if (data.length > 0) {
					...
					uni.setStorageSync("fc3d", this.placeData)
