<template>
  <div class="app-container">
    ...
    <el-table ref="table" v-if="refreshTable" v-loading="loading" :data="voList" border stripe row-key="id"
        ...
        <template slot-scope="scope">
          <el-card>
            <el-form label-position="left" inline class="demo-table-expand">
              ...
              <el-row>
                <el-col :span="12" v-if="scope.row.state=='0'">
                  <el-form-item>
                    <template>
                      <el-button size="mini" type="success" @click="process(scope.row.id,1)">审核通过</el-button>
                      <el-button size="mini" type="warning" @click="process(scope.row.id,2)">拒绝</el-button>
                    </template>
                  </el-form-item>
                </el-col>
              </el-row>
      ...
      <el-table-column label="提现金额" align="center" prop="amount" width="100" :show-overflow-tooltip="true" />
...
<script>
  ...
  export default {
    ...
    methods: {
      process(id, state) {
        this.loading = true
        let stateName = state == 1 ? '打款' : '拒绝'
        this.$confirm('确定要 ' + stateName + ' 吗？', '提示', {
          ...
          processWithdrawal(data).then((res) => {
            this.loading = false
            if (res.success) {
              this.handleQuery()
            }
